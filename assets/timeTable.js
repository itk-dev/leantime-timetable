    ...
    class TimeTable {
        ...
        registerEventHandlers() {
            ...
            document.addEventListener("mousedown", function (event) {
                ...
                if ($(this.entryCopyModal).is(':visible') && !this.entryCopyModal[0].contains(event.target)) {
            ...
            this.entryCopyButton.click((e) => {
                ...
                const { elementsWithoutValue, totalElements } = this.handleHighlighting(eventTarget);
                ...
                const formattedCopyFromDate = new Date(copyFromDate).toLocaleDateString('da-DK', {
                    day: 'numeric',
                    month: 'numeric',
                }).replace('.', '/');
                ...
                const formattedDayAfterCopyFromDate = dayAfterCopyFromDate.toLocaleDateString('da-DK', {
                    day: 'numeric',
                    month: 'numeric',
                }).replace('.', '/');;
                ...
                const formattedCopyToDate = new Date(copyToDate).toLocaleDateString('da-DK', {
                    day: 'numeric',
                    month: 'numeric',
                }).replace('.', '/');

                this.entryCopyForm.find('input[name="entryCopyTicketId"]').val(ticketId);
                this.entryCopyForm.find('input[name="entryCopyHours"]').val(hours);
                this.entryCopyForm.find('input[name="entryCopyDescription"]').val(description);
                this.entryCopyForm.find('input[name="entryCopyFromDate"]').val(copyFromDate);
                this.entryCopyForm.find('input[name="entryCopyToDate"]').val(copyToDate);
                ...
                this.entryCopyCheckboxOverwrite.off("change").change((e) => {
                    ...
                    const elementsCount = overwrite ? totalElements : elementsWithoutValue;
        ...
        handleHighlighting(element, overwrite = false) {
            ...
            elements.each(function (index, el) {
                ...
                const shouldHighlight = overwrite || (span.length > 0 && span.text().trim() === "");
                if (shouldHighlight) {
                    setTimeout(() => {
        ...
        setEntryCopyText({ formattedCopyFromDate, formattedDayAfterCopyFromDate, formattedCopyToDate, elementsWithoutValue }) {
            this.entryCopyForm.find('.entry-copy-headline').html(`<b>Kopier tidslog fra d. ${formattedCopyFromDate}</b>`);
            this.entryCopyForm.find('.entry-copy-text').html(`Til d. ${formattedDayAfterCopyFromDate} til og med d. ${formattedCopyToDate}<br>${elementsWithoutValue} ${elementsWithoutValue === 1 ? 'dag bliver ændret' : 'dage bliver ændret'}`);
        }
